...
The configuration is based on the processing architecture.
...
The Project ID is configuration information rather than an authentication credential. Storing it in the working-directory configuration allows different processing directories to use different Earth Engine projects if required.

Setting the value directly does not authenticate Google Earth Engine.
...
The value is an angular geographic buffer. It is not a fixed distance in kilometres because the ground distance represented by one degree of longitude varies with latitude.
...
The configured cycle list can be inspected with:
...
A lower threshold removes observations at smaller deviations from the median, while a higher threshold permits larger deviations.
...
Controls the geographic buffer applied around the reservoir bounds during initial PIXC metadata discovery.

By default, the reservoir bounding box is expanded by 0.5° in each geographic direction before the product search is performed.

As with the LakeSP search buffer, this is an angular geographic buffer rather than a fixed distance in kilometres.
...
For **PIXC**, it provides the temporary workspace for downloading and processing PIXC NetCDF products. PIXC granules are currently processed through temporary workspaces rather than being retained in a persistent product cache.
...
| sources.lakesp.science_cycles | Configured science-cycle list | LakeSP cycle filtering |
...
| sources.pixc.science_cycles | Configured science-cycle list | PIXC cycle filtering |
| sources.pixc.mad_threshold | 3.0 | PIXC temporal filtering |
| sources.pixc.water_classification | 4 | PIXC pixel selection |

The active LakeSP and PIXC science-cycle lists can be inspected with:
